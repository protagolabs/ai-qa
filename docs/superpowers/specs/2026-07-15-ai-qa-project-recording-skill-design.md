...
**2026-07-16 取代通知：**
`2026-07-16-ai-qa-host-managed-project-skill-design.md` 已取代本文件中由
CLI author/preview/apply Project Skill 的設計、初始化 transaction、managed/user
regions、embedded checksum，以及 project `skill generate|sync` surface。以下舊章節保留
為設計歷史，不是目前實作指示。Provider-neutral recording mode、per-run mode
snapshot、verified-report boundary、中立 receipt、crash recovery 與 QA verdict
immutability 仍有效，除非 7 月 16 日規格另有明文修改。
...
> **已由 2026-07-16 規格取代：** 本章的 CLI-owned authoring、combined
> initialization request、preview checksum、transaction publish 與 managed/user
> merge 均不再是目前流程。現行流程由 Codex 使用 `skill-creator` 在 scratch space
> 建立普通、project-owned Skill，分別驗證 config/Skill，顯示完整 diffs、取得一次
> 確認後由 host 寫檔。以下內容只保留歷史脈絡。
...
> **歷史、已取代：** 現行設計沒有 `InitializationRequest.projectSkill.content`、
> init preview checksum 或 CLI transaction apply。
...
> **歷史、已取代：** 現行 Project Skill 是普通的 project-owned Skill，沒有
> AI-QA managed/user markers 或 embedded checksum。
...
> **部分已由 2026-07-16 規格取代：** 下列 project `init`、`configure`、
> `skill generate|check|sync --project` mutation surface 已移除。Global main-Skill
> `skill install|check|sync --global` 與 provider-neutral report receipt/status surface
> 仍保留。以下清單記錄 7 月 15 日的舊設計。
