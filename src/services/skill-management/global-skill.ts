...
export async function checkGlobalSkillForProject(input: {
  ...
  try {
    ...
    const currentBundledVersion =
      metadata.aiQaSkillVersion === sourceMetadata.aiQaSkillVersion;
    const safeLegacyLocalOnly =
    ...
    let expectedReferences = references;
    if (safeLegacyLocalOnly) {
      ...
      const [legacySource, legacyReferences] = await Promise.all([
        readFile(legacySourcePath, "utf8"),
        readReferenceAssets(legacySourcePath),
      ]);
      const legacyMetadata = parseInstalledMetadata(legacySource, {
        allowMissingReceiptCapability: true,
      });
      const legacyInspection = inspectManagedSkill(legacySource);
      if (
        legacyMetadata.aiQaSkillVersion !== metadata.aiQaSkillVersion ||
        legacyInspection.managedChecksum !== inspection.managedChecksum
      ) {
        return { status: "stale", destination };
      }
      expectedReferences = legacyReferences;
    }
