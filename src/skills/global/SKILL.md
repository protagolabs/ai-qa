...
6. Use the fixed description template `Use when performing <Project Name> Web AI QA.` by replacing only `<Project Name>`; add no suffix. Build the rest of `projectSkill.content` with the canonical Project Skill wire format in the reference, including compatible metadata, managed and user markers, and a computed managed checksum; prose-only Skill content is not an initialization payload. Before presenting the request, actually execute the managed-checksum algorithm over its final bytes and verify the embedded value; never claim an unverified checksum. Run the complete request through production `initializationRequestSchema` and `prepareProjectSkill()` before presenting it.
